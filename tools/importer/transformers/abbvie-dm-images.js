...
/**
 * Transformer: AbbVie Dynamic Media / Scene7 images.
 *
 * The source uses Scene7 IS/Image URLs (https://abbvie.scene7.com/is/image/...);
 * 13 such URLs are present in migration-work/metadata.json .images.mapping.
 * EDS's path-only optimizer would strip the Scene7 query parameters, so instead
 * we convert every DM <img> into an anchor that round-trips through markdown
 * intact. A companion auto-block (buildDynamicMediaImages in scripts/scripts.js,
 * installed by the site-migration orchestrator) rebuilds responsive <picture>
 * markup from these anchors at render time.
 *
 ...
 * rewriting imgs to anchors earlier would leave those parsers with empty cells.
 *
 * Helpers below are copied byte-identical from the canonical
 * sub-agents/.../references/dm-scene7-helpers.js (transformer subset). Do not
 * re-derive the detection regex or wrapper-walk logic.
 */
